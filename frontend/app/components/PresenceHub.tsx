...
function positionOnArc(
  ...
  // Arc from 180° to 270° (bottom-left quadrant, opening upward + rightward)
...
function UserOrb({
  ...
  useEffect(() => {
    ...
    if (user.isSpeaking) {
      gsap.to(orbRef.current, {
        boxShadow: `0 0 0 3px ${user.color}55, 0 0 16px ${user.color}88`,
        duration: 0.2,
      });
    } else {
      gsap.to(orbRef.current, {
  ...
  return (
    ...
    >
      ...
      {/* Tooltip */}
      <div
        className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity"
      ...
      >
        ...
        >
          ...
          {user.email && (
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 10 }}>{user.email}</p>
          )}
        </div>
      </div>
    </div>
...
export default function PresenceHub({ activeUsers, currentUserId }: Props) {
  ...
  const openHub = useCallback(() => {
    ...
    if (sectorRef.current) {
      tl.fromTo(
        sectorRef.current,
        { opacity: 0, scale: 0.4, transformOrigin: "100% 100%" },
    ...
    orbRefs.current.forEach((el, i) => {
      if (!el) return;
      const { x, y } = positionOnArc(i, activeUsers.length, RADIUS);
      tl.fromTo(
        el,
  ...
  const closeHub = useCallback(() => {
    ...
    orbRefs.current.forEach((el, i) => {
      ...
      tl.to(
        el,
        { x: 0, y: 0, scale: 0, opacity: 0, duration: 0.25, ease: "back.in(1.4)" },
        i * 0.04
      );
    });
    ...
    if (sectorRef.current) {
      tl.to(
        sectorRef.current,
        { opacity: 0, scale: 0.4, transformOrigin: "100% 100%", duration: 0.3, ease: "power2.in" },
        0.1
      );
    }
  }, []);
  ...
  // Re-run open animation when users change while open
  useEffect(() => {
    if (isOpen) openHub();
  ...
  // ── Merge current user volume into their orb ───────────────────────────────
  const enrichedUsers = activeUsers.map((u) =>
    u.userId === currentUserId
      ? { ...u, volume: myVolume, isSpeaking: myVolume > 0.15 }
      : u
  );

  // ── SVG sector path (bottom-left quadrant arc) ────────────────────────────
  // Drawn in local coords where origin is bottom-right of the SVG (where hub sits)
  const SVG_SIZE = RADIUS + 70;
  // Arc from angle 180° to 270°, radius = RADIUS + 24 (slightly bigger than orb orbit)
  const arcR = RADIUS + 28;
  // In our coordinate system: origin at (SVG_SIZE, SVG_SIZE)
  // 180° → left: (-arcR, 0) → (0, SVG_SIZE)
  // 270° → down: (0, arcR) → (SVG_SIZE, SVG_SIZE + arcR) … but SVG up is negative y
  // Let's compute properly:
  const toSVG = (angleDeg: number, r: number) => {
    const rad = (angleDeg * Math.PI) / 180;
    return {
      x: SVG_SIZE + Math.cos(rad) * r,
      y: SVG_SIZE + Math.sin(rad) * r,
    };
  };
  const p180 = toSVG(180, arcR);
  const p270 = toSVG(270, arcR);
  const sectorPath = `
    M ${SVG_SIZE} ${SVG_SIZE}
    L ${p180.x} ${p180.y}
    A ${arcR} ${arcR} 0 0 1 ${p270.x} ${p270.y}
    Z
  `;

  const others = enrichedUsers.filter((u) => u.userId !== currentUserId);
  const me = enrichedUsers.find((u) => u.userId === currentUserId);
  ...
  return (
    ...
    >
      ...
      <svg
        ...
        viewBox={`0 0 ${SVG_SIZE + 10} ${SVG_SIZE + 10}`}
      >
        <defs>
          <radialGradient id="sectorGrad" cx="100%" cy="100%" r="100%">
            <stop offset="0%" stopColor="rgba(255,255,255,0.07)" />
            <stop offset="60%" stopColor="rgba(255,255,255,0.03)" />
            <stop offset="100%" stopColor="rgba(255,255,255,0)" />
          </radialGradient>
          <filter id="sectorBlur">
        ...
        <path
          ...
          strokeWidth="1"
          filter="url(#sectorBlur)"
          style={{ backdropFilter: "blur(20px)" }}
        />
        {/* Subtle arc guide line */}
        <path
          d={`M ${p180.x} ${p180.y} A ${arcR} ${arcR} 0 0 1 ${p270.x} ${p270.y}`}
          fill="none"
      ...
      >
        {/* Stacked avatar preview OR open icon */}
        {!isOpen ? (
      ...
      {/* Tooltip hover CSS */}
      <style>{`
        .group:hover .orb-tooltip { opacity: 1 !important; }
      `}</style>
    </div>
  );
}